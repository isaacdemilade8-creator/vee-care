...
import type { AdminUserInvitationResult, PendingAdminUserInvitation, Role } from '../types';
...
/**
 * Hospital user directory for hospital administrators. Lists the tenant's own
 * users (never another hospital's) with server-side search, role/status
 * filtering and pagination, plus outstanding invitations. Inviting returns a
 * single-use token that is shown exactly once.
 */
export function AdminUsersPage() {
  ...
  const [invitation, setInvitation] = useState<AdminUserInvitationResult | null>(null);
  const [copiedKey, setCopiedKey] = useState<string | null>(null);
  const [pendingRevoke, setPendingRevoke] = useState<PendingAdminUserInvitation | null>(null);
  ...
  // the tenant SPA page on the same host; that page then calls the (POST-only)
  // API internally.
  ...
  const inviteMutation = useMutation({
    mutationFn: async (payload: { name: string; email: string; role: Role }) => {
      const response = await endpoints.inviteAdminUser(payload);
  ...
  const revokeMutation = useMutation({
    mutationFn: async (id: number) => {
      await endpoints.revokeAdminUserInvitation(id);
    },
    onSuccess: () => {
    ...
    <div className={styles.page}>
      <header className={styles.heading}>
        <div>
          ...
          <h1>Users</h1>
          <p>Manage patients, clinical and staff accounts within this hospital.</p>
        </div>
      ...
      {users.isLoading ? (
        ...
        <DataStatePanel
          title={search || role || status ? 'No users match your filters' : 'No users yet'}
          description={search || role || status ? 'Try adjusting the search or filters.' : 'Invited users appear here once they join.'}
        />
      ) : (
          ...
          <div className={styles.table}>
            ...
            {rows.map((user) => (
              <div className={styles.row} role="row" key={user.id}>
                <span role="cell">
                  <Link to={`/admin/users/${user.id}`} className={styles.name}>
                    {user.name}
                  </Link>
                ...
                <span role="cell" className={styles.actionsCol}>
                  <Link to={`/admin/users/${user.id}`} className={styles.view}>
                    <Eye size={15} />
                    View
                  </Link>
                </span>
            ...
            {pending.map((invite) => (
              <div className={styles.pendingRow} key={invite.id}>
                <div className={styles.pendingMain}>
                  ...
                  <span>
                    {tenantRoleLabel[invite.role]} · invited by {invite.inviterName ?? 'System'} ·
      ...
      {inviteOpen ? (
        <Modal title="Invite a hospital user" onClose={() => setInviteOpen(false)}>
          ...
          <div className={styles.form}>
            ...
            <TextField
              ...
              placeholder="e.g. jordan@hospital.com"
              maxLength={255}
