...
import styles from './HospitalAdminInvitationAcceptPage.module.scss';
...
/**
 * Public hospital-administrator invitation acceptance.
 *
 * Reached only via the single-use, expiring invitation link embedded in the
 * setup email (/platform/applications/invitations/:token/accept). The raw
 * token is read from the route, sent once with the POST to the existing public
 * endpoint, and is never persisted, displayed or logged. The password is set
 * server-side and never returned.
 */
export function HospitalAdminInvitationAcceptPage() {
  ...
  const accept = useMutation({
    mutationFn: async (values: AcceptForm) => {
      const response = await platformEndpoints.acceptHospitalInvitation(token ?? '', values);
        ...
        <div className={authStyles.form}>
          ...
          {acceptedEmail ? (
            <div className={styles.stateCard} role="status">
              ...
              <h1>Hospital administrator set up</h1>
              <p>
                Your hospital administrator account for <strong>{acceptedEmail}</strong> is ready.
            ...
            <form className={authStyles.fields} onSubmit={handleSubmit(onSubmit)}>
              <h1>Set up your hospital account</h1>
              <p>
                You have been invited to be the hospital administrator for your organisation on
                Vee-Care. Set a password to activate your account.
              </p>
